...
class FieldBase(SQLModel):
    ...
    default: Optional[int] = Field(
        default=None, sa_column=Column(String(32), comment="默认值")
    )
    length: Optional[int] = Field(
        default=None, sa_column=Column(Integer, comment="长度")
    )
    decimals: Optional[int] = Field(
        default=None, sa_column=Column(Integer, comment="小数位数")
    )
    not_null: Optional[int] = Field(
        default=1, sa_column=Column(Integer, comment="允许为空(0:允许, 1:不允许)")
    )
    key: Optional[int] = Field(
        default=0, sa_column=Column(Boolean, comment="是否主键(0:否, 1:是)")
    )
