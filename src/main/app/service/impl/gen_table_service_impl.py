...
class GenTableServiceImpl(ServiceBaseImpl[GenTableMapper, GenTableDO], GenTableService):
    ...
    async def import_gen_table(self, table_import: TableImport):
        ...
        for table_record in table_records:
            ...
            gen_table_record = GenTableDO(database_id= table_import.database_id, db_table_id=table_id, class_name = table_name, function_name = comment, comment=comment, table_name=table_record.name, backend=backend)
            ...
            for field_record in field_records:
                gen_field_record = GenFieldDO(db_field_id=field_record.id, db_table_id=field_record.table_id, length=field_record.length)
                GenUtils.init_field(gen_field_record, field_record, backend)
                await genFieldMapper.insert(record=gen_field_record)

    async def preview_code(self, gen_table_id: int) -> Dict:
        data_map = OrderedDict()
        # 查询导入的表信息
        gen_table, table_gen = await self.generator_code(gen_table_id)
        index_metadata = await indexMapper.select_by_table_id(table_id=gen_table.db_table_id)
        backend = gen_table.backend
        data_map["backend"] = backend
        context = Jinja2Utils.prepare_context(table_gen, index_metadata)
        templates = Jinja2Utils.get_template_list(backend, gen_table.tpl_backend_type, gen_table.tpl_category, gen_table.tpl_web_type)
